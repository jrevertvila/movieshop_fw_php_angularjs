...
class shop_dao {
    ...
    public function getAllMovies($db) {
        $sql = 'SELECT *,(SELECT COUNT(*) FROM user_favorites_movies WHERE id_movie=f.id) as favs FROM films f';
    ...
    public function getAllMoviesByGenres($db,$data) {
        $sql = 'SELECT DISTINCT f.*,(SELECT COUNT(*) FROM user_favorites_movies WHERE id_movie=f.id) as favs FROM films f INNER JOIN films_genres r ON r.id_film = f.id and r.id_genre in ('.$data.')';
        $stmt = $db->ejecutar($sql);
        return $db->listar($stmt);
    }
